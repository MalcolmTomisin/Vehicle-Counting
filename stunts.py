...
blob_id = 1
frame_counter = 0
DETECTION_FRAME_RATE = 75
MAX_CONSECUTIVE_TRACKING_FAILURES = 5
...
async def init_trackers():
    grab, container = await input_stream()
    initial_bboxes = get_bounding_boxes(container)
    for box in initial_bboxes:
        tracker = cv2.TrackerCSRT_create()
        tracker.init(container, tuple(box))
        _blob = Blob(box, tracker)
        blobs[blob_id] = _blob
        f_height, f_width, grab = container.shape
        global cl_y
        cl_y = round(4 / 5 * f_height)
...
asyncio.run(init_trackers())


async def update_trackers(grab, container):
                        for _id, blob in list(blobs.items()):
                            global box
                            success, box = blob.tracker.update(container)
                            if success:
                                ...
                                cv2.rectangle(
                                    container, (x, y), (x + w, y + h), (0, 255, 0), 2)
                                cv2.putText(container, 'v_' + str(_id), (x, y - 2),
...
while True:
    ...
    if cap.get(cv2.CAP_PROP_POS_FRAMES) + 1 < cap.get(cv2.CAP_PROP_FRAME_COUNT):
        _, frame = cap.read()
        asyncio.run(update_trackers(_,frame))
